...
void Context::initBuffers()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, agents->size * sizeof(Agent), nullptr, GL_STREAM_DRAW);

    glUseProgram(agentProgram.id);
    glVertexAttribPointer(agentProgram.attr["position"], 3, GL_FLOAT, GL_FALSE, sizeof(Agent), 0);
    glEnableVertexAttribArray(agentProgram.attr["position"]);

    glVertexAttribPointer(agentProgram.attr["velocity"], 3, GL_FLOAT, GL_FALSE, sizeof(Agent), (void *) (3 * sizeof(float)));
...
{
    glBindBuffer(GL_ARRAY_BUFFER, posBuffer);
    glBufferSubData(GL_ARRAY_BUFFER, 0, agents->size * sizeof(Agent), agents->boids);
